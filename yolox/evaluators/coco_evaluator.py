...
class COCOEvaluator:
    ...
    def __init__(
        ...
        Args:
            ...
            img_size: image size after preprocess. images are resized
                to squares whose shape is (image_size, image_size).
    ...
    def evaluate(
        ...
        max_iter = len(self.dataloader) - 1
        n_samples = max(max_iter, 1)
        ...
        self.data_list.extend(self.convert_to_coco_format(outputs, info_imgs, ids))

        statistics = torch.cuda.FloatTensor([self.inference_time, self.nms_time, n_samples])
        if distributed:
            self.data_list = gather(self.data_list, dst=0)
            self.data_list = list(itertools.chain(*self.data_list))
            torch.distributed.reduce(statistics, dst=0)

        if cur_iter == max_iter:
            ap50_95, ap50, summary = self.evaluate_prediction(self.data_list, statistics)
            synchronize()
            pl_module.log("ap50_95", ap50_95)
            pl_module.log("ap50", ap50)
            logger.info('\n' + summary)
        # else:
        #     logger.info(f"On Validation step: {cur_iter}/{max_iter}")
